...
def apply_test_patch() -> None:
    ...
    if marker in text:
        raise SystemExit("threshold-source falsification test already present")

    anchor = "def test_u2_primary_cell_gate_evidence_rejects_pass_state_tampering() -> None:\n"
    block = '''def test_u2_primary_cell_gate_uses_preregistered_threshold_payload(
    monkeypatch,
) -> None:
...
'''
    text = _replace_once(
        ...
        anchor,
        block + anchor,
        label="threshold-source test insertion",
    )
...
def apply_source_patch() -> None:
    ...
    if "selection_thresholds_digest: str = field(init=False)" in text:
        raise SystemExit("contract-bound primary gate already present")
    ...
    start = text.find("U2_PRIMARY_SELECTION_CELL_GATE_SCHEMA: Final = (")
    end_marker = "@dataclass(frozen=True, slots=True)\nclass UniversalTradeRLU2PairedExcessPoint:"
    end = text.find(end_marker, start)
    if start < 0 or end < 0:
        raise SystemExit("existing primary gate block anchors are missing")
...
'''
    text = text[:start] + gate_block + text[end:]
