...
if "def build_universal_trade_rl_u2_development_selection_evidence(" in text:
    raise SystemExit("Task4 final Selection API already exists; refusing duplicate patch")
...
from trade_rl.workflows.universal_trade_rl_u2_predevelopment import (
    U2_ADMISSION_STATUS as U2_PREDEVELOPMENT_ADMISSION_STATUS,
    U2_PRODUCTION_STATUS as U2_PREDEVELOPMENT_PRODUCTION_STATUS,
...
class UniversalTradeRLU2DevelopmentSelectionEvidence:
    """Canonical final Development Selection decision for one frozen U2 generation."""

    primary_cell_gates: tuple[UniversalTradeRLU2PrimarySelectionCellGateEvidence, ...]
    seed_robustness: tuple[UniversalTradeRLU2SeedRobustnessEvidence, ...]
    checkpoint_closure: UniversalTradeRLU2FinalCheckpointClosure
    base_lock: UniversalTradeRLU2DevelopmentLock
    authoritative_lock: UniversalTradeRLU2AuthoritativeDevelopmentLock
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.primary_cell_gates, tuple):
        ...
        primary_cells = tuple(gate.cell for gate in gates)
        if primary_cells != _U2_FINAL_SELECTION_PRIMARY_CELLS:
            raise ValueError(
        ...
        for gate in gates:
            ...
            if gate.selection_thresholds_digest != current_selection_thresholds_digest:
                raise ValueError(
                    "U2 final Selection primary gate threshold identity drifted"
                )

        if not isinstance(self.seed_robustness, tuple):
            raise TypeError(
                "U2 final Selection robustness results must be an immutable tuple"
            )
        robustness = self.seed_robustness
        if any(
            not isinstance(result, UniversalTradeRLU2SeedRobustnessEvidence)
            for result in robustness
        ):
            raise TypeError("U2 final Selection robustness result is invalid")
        robustness_scopes = tuple(result.scope for result in robustness)
        if robustness_scopes != _U2_FINAL_SELECTION_ROBUSTNESS_SCOPES:
            raise ValueError(
                "U2 final Selection requires ordered D1/D2/D1+D2 robustness closure"
            )
        current_cross_seed_thresholds = _u2_cross_seed_robustness_thresholds()
        current_cross_seed_thresholds_digest = content_digest(
            current_cross_seed_thresholds
        )
        for result in robustness:
            _require_current_artifact_digest(
                result,
            ...
            if result.training_seeds != U2_TRAINING_SEEDS:
                raise ValueError(
                    "U2 final Selection robustness seed closure is not canonical"
                )
        ...
        if not isinstance(
            self.authoritative_lock,
            UniversalTradeRLU2AuthoritativeDevelopmentLock,
        ):
            raise TypeError("U2 final Selection authoritative lock is invalid")

        _require_current_artifact_digest(
            self.checkpoint_closure,
            field_name="U2 final Selection checkpoint closure",
        )
        _require_current_artifact_digest(
            self.base_lock,
            field_name="U2 final Selection base Development lock",
        )
        _require_current_artifact_digest(
            self.authoritative_lock,
            field_name="U2 final Selection authoritative Development lock",
        )
        ...
        if checkpoint_seeds != U2_TRAINING_SEEDS:
            raise ValueError(
                "U2 final Selection checkpoint closure must use exact seeds (0,1,2)"
            )
        if self.base_lock.checkpoint_digests != self.checkpoint_closure.checkpoint_digests:
            raise ValueError(
                "U2 final Selection checkpoint mapping identity drifted from base lock"
            )
        if self.authoritative_lock.base_lock_digest != self.base_lock.digest:
            raise ValueError(
                "U2 final Selection authoritative/base lock identity mismatch"
            )
        if (
            self.authoritative_lock.final_checkpoint_closure_digest
            != self.checkpoint_closure.digest
        ):
            raise ValueError(
                "U2 final Selection authoritative checkpoint closure identity mismatch"
            )

        if not (
            self.base_lock.predevelopment_contract_digest
            == self.checkpoint_closure.predevelopment_contract_digest
            == self.authoritative_lock.predevelopment_contract_digest
        ):
            raise ValueError(
        ...
        if not (
            ...
            == self.authoritative_lock.universe_manifest_digest
        ):
        ...
        if not (
            ...
            == self.authoritative_lock.u2_contract_digest
        ):
        ...
        if not (
            ...
            == self.authoritative_lock.u1_contract_digest
        ):
        ...
        if not (
            ...
            == self.authoritative_lock.u1_normalizer_digest
        ):
        ...
        if self.base_lock.development_numeric_open_count != 0:
            raise ValueError(
                "U2 final Selection requires the frozen Development lock open count"
            )
        if self.base_lock.admission_numeric_open_count != 0:
            raise ValueError(
                "U2 final Selection requires Admission to remain numerically unopened"
            )
        if (
            self.base_lock.admission_status != U2_PREDEVELOPMENT_ADMISSION_STATUS
            or self.authoritative_lock.admission_status
            != U2_PREDEVELOPMENT_ADMISSION_STATUS
        ):
            raise ValueError("U2 final Selection requires Admission to remain SEALED")
        if (
            self.base_lock.production_status != U2_PREDEVELOPMENT_PRODUCTION_STATUS
            or self.checkpoint_closure.production_status
            != U2_PREDEVELOPMENT_PRODUCTION_STATUS
            or self.authoritative_lock.production_status
            != U2_PREDEVELOPMENT_PRODUCTION_STATUS
        ):
        ...
        primary_checkpoint = checkpoint_by_seed.get(
        ...
        object.__setattr__(
            ...
            primary_checkpoint if passed else None,
        )
    ...
    def robustness_scopes(self) -> tuple[str, ...]:
        return tuple(result.scope for result in self.seed_robustness)

    @property
    def base_lock_digest(self) -> str:
        return self.base_lock.digest

    @property
    def authoritative_lock_digest(self) -> str:
        return self.authoritative_lock.digest

    @property
    def final_checkpoint_closure_digest(self) -> str:
        return self.checkpoint_closure.digest
    ...
    def to_payload(self, *, include_digest: bool = True) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "seed_robustness_digests": tuple(
                result.digest for result in self.seed_robustness
            ),
            "final_checkpoint_closure_digest": self.final_checkpoint_closure_digest,
            "base_lock_digest": self.base_lock_digest,
            "authoritative_lock_digest": self.authoritative_lock_digest,
            "passed": self.passed,
            "admission_eligible": self.admission_eligible,
            "selected_checkpoint_digest": self.selected_checkpoint_digest,
            "promotion_eligible": self.promotion_eligible,
            "admission_status": U2_PREDEVELOPMENT_ADMISSION_STATUS,
            "production_status": U2_PREDEVELOPMENT_PRODUCTION_STATUS,
        }
        if include_digest:
...
def build_universal_trade_rl_u2_development_selection_evidence(
    *,
    ...
    """Build canonical all-AND Development Selection evidence without opening Admission."""
...
workflow_path = Path(".github/workflows/u2-contracts.yml")
workflow = workflow_path.read_text(encoding="utf-8")
needle = "          tests/workflows/test_universal_trade_rl_u2_selection_robustness.py\n"
count = workflow.count(needle)
if count != 3:
    raise SystemExit(f"expected 3 robustness CI anchors, found {count}")
