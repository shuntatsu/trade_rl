...
TARGET_SHA = "d1bf25fd66834289ffddb36063b0d5cef649f444"
BASE_SHA = "d18434799651cfc6c07e0840c40600dcf1dfa763"
VERIFICATION_RUN_ID = 35213065827
PR_NUMBER = 633
...
def _api(path: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise SystemExit(f"GitHub object expected: {path}")
    return value
...
def build(output_dir: Path) -> None:
    ...
    for name, expected_value in expected.items():
        if getattr(spec, name) != expected_value:
    ...
    if changed != DURABLE_FILES:
        raise SystemExit(f"durable diff drift: {changed!r}")
    payload = {
        "schema_version": "issue630_shared_cash_evaluator_final_implementation_v1",
        "issue_number": 630,
    ...
    seal_payload = {
        "schema_version": "issue630_shared_cash_evaluator_final_seal_v1",
        "implementation_head": TARGET_SHA,
...
def audit(primary: Path, fresh: Path, closure: Path) -> None:
    primary_impl = (primary / "implementation.json").read_bytes()
    fresh_impl = (fresh / "implementation.json").read_bytes()
    primary_seal = (primary / "seal.json").read_bytes()
    fresh_seal = (fresh / "seal.json").read_bytes()
    if primary_impl != fresh_impl or primary_seal != fresh_seal:
        raise SystemExit("primary/fresh bytes differ")
    ...
    if (
        ...
        raise SystemExit("PR 633 authority drift")
    ...
    for name in ("PRIMARY", "FRESH"):
        ...
        if (
            ...
            raise SystemExit(f"{name.lower()} seal artifact drift")
        own[name.lower()] = (artifact_id, digest)
    ...
    if seal.get("implementation_sha256") != implementation_sha:
        raise SystemExit("sealed implementation digest drift")
    ...
    closure.write_bytes(
        _canonical(
            {
                "schema_version": "issue630_shared_cash_evaluator_final_seal_closure_v1",
                "seal_run_id": run_id,
                "implementation_head": TARGET_SHA,
                "implementation_sha256": implementation_sha,
                "seal_sha256": seal_sha,
                "primary_artifact_id": own["primary"][0],
                "primary_api_digest": own["primary"][1],
                "fresh_artifact_id": own["fresh"][0],
                "fresh_api_digest": own["fresh"][1],
...
def main() -> None:
    ...
    audit_parser.add_argument("--primary", type=Path, required=True)
    audit_parser.add_argument("--fresh", type=Path, required=True)
    audit_parser.add_argument("--closure", type=Path, required=True)
    args = parser.parse_args()
    if args.command == "build":
        build(args.output_dir)
    else:
        audit(args.primary, args.fresh, args.closure)
