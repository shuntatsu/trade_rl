"""Independent no-refit audit for the official Issue #645 directional study."""
...
ISSUE_NUMBER = 645
TARGET_SHA = "928ee0886c404cdbb1383f440f731d7221c277ef"
ACTIVATION_NAME = "issue645-corrected-directional-activation-v1"
PROTOCOL_NAME = "issue645-corrected-directional-protocol-v1"
SUMMARY_NAME = "issue645-corrected-directional-run-summary-v1"
OFFICIAL_REF = "run/issue645-corrected-directional-study-v1"
...
def _load_activation_helper(target: Path) -> ModuleType:
    del target
    path = Path(__file__).with_name("issue645_directional_activation.py")
    spec = importlib.util.spec_from_file_location(
        "issue645_directional_activation", path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Issue 645 activation helper cannot be loaded")
...
def audit(
    *,
    ...
    for arm in ARMS:
        package = arms_root / f"issue645-corrected-directional-arm-{arm}-v1"
    ...
    expected_names = [
        ...
        PROTOCOL_NAME,
        SUMMARY_NAME,
        *(f"issue645-corrected-directional-arm-{arm}-v1" for arm in ARMS),
    ]
    ...
    payload = {
        "schema_version": "issue645_corrected_directional_official_audit_v1",
