...
if [[ "$#" -ne 1 ]]; then
  echo "usage: issue645_official_run_arm.sh <arm>" >&2
  exit 2
fi
...
export ISSUE645_ACTIVATION_HELPER="$script_dir/issue645_directional_activation.py"
...
if not protocol_path.is_file() or not claim_path.is_file():
    raise SystemExit("Issue 645 activation/protocol evidence is missing before arm")
...
if canonical_json_bytes(protocol) != protocol_raw or canonical_json_bytes(claim) != claim_raw:
    raise SystemExit("Issue 645 activation/protocol evidence is noncanonical")
activation = protocol.get("official_activation")
if not isinstance(activation, dict):
    raise SystemExit("Issue 645 official activation is missing from protocol")
if (
    ...
    raise SystemExit("Issue 645 official activation identity drifted before arm")
path = Path(os.environ["ISSUE645_ACTIVATION_HELPER"])
spec = importlib.util.spec_from_file_location("issue645_activation", path)
if spec is None or spec.loader is None:
    raise SystemExit("Issue 645 activation helper cannot be loaded")
...
print(f"ISSUE645_ARM_{arm}_ACTIVATION_REBOUND=true")
...
payload = {
    "schema_version": "issue645_corrected_directional_arm_attempt_v1",
    "issue_number": 645,
    "arm": arm,
    "workflow_run_id": int(os.environ["GITHUB_RUN_ID"]),
...
echo "ISSUE645_ARM_${arm}_ATTEMPT_RECORDED=true"
