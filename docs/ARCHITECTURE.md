...
Selected-final packaging also requires `paper_reconciliation_evidence_v1`. The evaluation layer derives its pass state from complete terminal-order coverage, complete fill matching, zero unknown or duplicate fills, zero open orders, and bounded position-notional, cash, and equity differences. Release packaging rejects declared tolerances above `1e-6`, requires the artifact digest, order/fill log digests, selected-final training identity, dataset, environment, policy, and collection interval to match the signed fresh confirmation, and copies the verified report into immutable bundle file closure. The artifact validator is an integrity capability; production remains `NO-GO` until a real maintained paper run supplies passing evidence.
