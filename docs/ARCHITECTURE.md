...
serving -> rl actions / observations / market inputs / artifacts / domain
...
`RawMarketSeries.timestamp` is the market event time. Optional `available_at` is the earliest time at which the source row was knowable and defaults to the event time. It may not precede the event time. A row with `available_at > timestamp` is retained as realized market history but is not injected into same-time feature, cross-sectional, tradability, or policy-input calculations. The last causal feature may be carried forward with increasing normalized staleness until a new on-time feature event becomes available.

`MarketDataset` carries:

- ordered symbols and timestamps;
- OHLCV and funding arrays;
- point-in-time `symbol_active`, realized `tradable`, `information_available`, and `available_at` arrays;
...
- feature-configuration, normalization, and canonical identity payloads.

Execution truth and observable state are intentionally distinct. `MarketDataset.observable_tradable(t)` returns `tradable[t] & information_available[t]`; only `MarketExecutor` consumes the unmasked realized tradability array.
...
## Point-in-time eligibility and market inputs
...
`MarketInputResolver` is the single maintained Trend/Alpha path. A causal Alpha provider receives a copied, read-only `CausalMarketView` ending at the current decision index. The view exposes feature and mask prefixes but no future rows or raw OHLC arrays. Resolver identity covers Trend configuration, whether Alpha is enabled, and the Alpha provider digest.
...
strict JSON build request
  ...
  -> verified manifest.json + deterministic arrays.npz
  -> one immutable directory rename
```

`trade-rl data build --config ... --output ...` resolves relative source paths against the build-request file. Unknown JSON fields and invalid enum or datetime values are rejected. The destination must not already exist and is never overwritten.

`manifest.json` binds ordered schemas, periodization, volume semantics, the SHA-256 digest of `arrays.npz`, and the complete canonical dataset identity payload. Loading verifies the artifact digests and independently recomputes `dataset_id` from the stored payload and arrays. A caller cannot make a different dataset valid merely by rewriting the ID string and outer manifest digest.
...
`ObservationBuilder` is the only maintained structured policy-input builder. Training and Serving call the same implementation. A decision at row `t` includes only row `t` observable market state, current book state and deterministic targets computed from history ending at `t`.

Per-symbol observations contain feature values, per-feature availability, per-feature staleness, current observable tradability and active state, fast/base/slow trend targets, optional alpha, hybrid and shadow weights, and their difference. The observation never includes `tradable[t + 1]`, delayed same-time tradability, or any other unavailable next-row value.
...
A serving bundle binds `dataset_id`, action schema, observation schema digest, exact observation size, and market-input resolver digest. `predict_state()` rejects a mismatched structured state and recomputes Trend and Alpha instead of trusting caller-supplied values. Raw-vector `predict()` requires matching dataset, observation-schema, and market-input identities in addition to exact size. A concurrent hot swap detected between validation and policy invocation also fails closed.
...
CI runs Ruff, formatting checks, mypy, Import Linter, Vulture advisory reporting, unit and property-based tests, migration tests and branch coverage. Regression tests cover prefix invariance under multiple independent future mutations, delayed current-row tradability isolation, current target independence from next-bar tradability, point-in-time Trend and causal Alpha eligibility, semantic dataset-ID reconstruction, immutable atomic dataset publication, serving dataset/schema/market-input/size guards, explicit volume conversion and training/Serving observation parity. The architecture contract also verifies that legacy execution trees and direct-action mode are absent.
