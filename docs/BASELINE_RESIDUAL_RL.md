...
- B is eligible only when it adds positive development excess return without exceeding the drawdown slack.
- C is never selected as a release policy; it measures whether the frozen alpha sleeve has standalone value.
- D is selected only when it is eligible and strictly beats both B and C.
- Otherwise the runner falls back to A (`baseline_only`).

The selected configuration is then evaluated once on the final test interval under 1x and 2x costs.
...
- `residual_alpha.json` — frozen residual-alpha model and data identity;
- `B_trend_mix_model.zip` or `B_trend_mix_ensemble/`;
- `D_combined_model.zip` or `D_combined_ensemble/` when the alpha gate passes;
- `residual_train_report.json` — A/B/C/D development results, frozen selection, final relative evaluation, cost-2x result, gates, and diagnostics;
- `residual_model_manifest.json` — dataset and training identity.
...
`baseline_only` bundles contain no PPO model. Serving supplies the identity action and runs the same trend, alpha, composer, HTF, post-processing, guardrail, and risk path. Schema mismatch, missing timestamps, missing alpha artifact, invalid feature order, or an incompatible model layout fails closed.
