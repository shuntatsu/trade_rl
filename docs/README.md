...
Active PPO improvement: [risk-alignment design](specs/ppo-risk-alignment.md) and
[implementation plan](plans/ppo-risk-alignment.md).
...
Directional studyのspec / implementation planは冒頭のリンクから参照する。新しい未実装designや実装中の独立planが必要になった場合だけ `specs/` / `plans/` を作り、完了後は恒久契約へ昇格してcurrent treeから削除する。
