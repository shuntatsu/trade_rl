...
- effective `tick_size > 0` のとき、LIMITの `limit_price` と STOP_MARKETの `stop_price` はそのpoint-in-time tick gridのexact decimal multipleでなければadmissionでfail closedする。off-tick boundを後段のexecution-price roundingで別価格へ自動変換しない。`tick_size == 0` は従来どおりprice grid未指定として扱う。
