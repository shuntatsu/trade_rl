...
The authoritative workflow matrix additionally exercises real nested walk-forward training, per-seed checkpoint selection, a fixed three-seed deployable ensemble, sealed outer-test evaluation, immutable ArtifactStore publication, signed serving-bundle activation, and deterministic inference.
...
| Three-seed sequence ensemble | PASS | real fold training for seeds 0/1/2, per-seed checkpoint selection, exact ensemble selection, sealed outer test, and publication |
...
3. Each of seeds 0, 1, and 2 produces checkpoint evidence.
4. The deployable mean ensemble contains exactly the selected seed members.
5. The exact ensemble—not a representative member—is evaluated on the sealed outer test.
