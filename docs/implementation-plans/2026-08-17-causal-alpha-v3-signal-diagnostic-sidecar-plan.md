...
- Produces: maintained pipeline behavior with paired persistence. The maintained `signal_scope_builder` injection point returns `CausalAlphaV3SignalScopeBuild`; existing tests that inject a custom builder are updated to return the paired type rather than bypassing diagnostic generation.
