...
The metric generated by the paired path must be byte/digest-equivalent to the metric generated by the existing path for the same inputs.
...
1. Fresh Signal scope computation produces the same canonical V2 metric/digest as the existing implementation and one deterministic diagnostic sidecar.
...
- exact equality of old-vs-new canonical Signal metric payload/digest for controlled inputs;
...
- byte-stable deterministic sidecar output for identical inputs;
