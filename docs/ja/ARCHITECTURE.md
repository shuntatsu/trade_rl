...
## Residual Walk-Forward研究境界

Residual Walk-Forwardは、research-onlyのControl Plane orchestrationです。`mars_lite.pipeline.residual_walk_forward`がデータ構築、解決済み不変設定、fold-local alpha fit、A/B/C/D候補の学習・選択、outer OOS実行、artifact staging、atomic publicationを所有します。`mars_lite.eval.residual_walk_forward`には、fold構築・stitched OOS統計・厳格なreport整形のpure evaluation primitivesだけを置き、pipeline layerをimportしてはいけません。

各foldはpolicy train、checkpoint validation、configuration selection、outer OOSをpurgeで分離します。Checkpoint validationとconfiguration selectionは重複させません。at least two completed foldsを満たさないrunはfail-closedで失敗し、新しい成功reportを公開しません。

Artifactはrun固有staging directoryへ書きます。成功時は`residual_wf_runs/<run_id>/`へ原子的に移動し、その後にだけ`residual_walk_forward.json`をatomic publicationします。失敗したpartial runは`failed/<run_id>/`へ隔離し、以前の成功runへ混在させません。

正規集計は、時系列順で重複しないbase-bar hybrid／shadow returnを連結したstitched OOSです。Fold平均・中央値は補助統計であり、Walk-Forward全体のtotal performanceではありません。Model provenanceはcontent SHA-256へ拘束し、1倍・2倍cost評価は同じmodel digestを参照しなければなりません。

この経路はresearch-onlyです。sealed release evidenceではなく、Registry登録を許可せず、Production判断は**NO-GO**のままです。
...
- `mars_lite/eval` — pure evaluation primitivesとreplay統計
