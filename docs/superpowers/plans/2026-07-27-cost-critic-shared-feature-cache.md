...
**Architecture:** During rollout collection, temporarily capture the exact feature tensor returned by the policy's own `extract_features` call and pass a detached view to the Cost Critic instead of invoking the extractor again. After the reward actor/value update, materialize one detached full-rollout feature tensor in policy evaluation mode. Every Cost Critic minibatch and diagnostic pass uses indexed rows from that immutable tensor. The cache exists only inside one Cost Critic update and is never serialized.
...
- Capture exactly one feature-extractor result per policy forward. Missing or ambiguous captures fail closed.
...
- [ ] Write a RED test that runs one four-step Cost Critic PPO update and counts `policy.extract_features` calls. With one PPO epoch and any number of Cost Critic epochs, the optimized contract is exactly seven calls: five policy evaluations during rollout/final bootstrap, one PPO minibatch evaluation, and one full-rollout Cost Critic cache build.
...
### Task 3: Implement rollout feature capture
...
- [ ] Use the helper for normal rollout policy forward, time-limit terminal value bootstrap, and final value bootstrap.
- [ ] Feed the captured detached tensor directly into `self.cost_critic` for cost-value prediction.
