...
Approved for implementation from the prior reward-design review. This specification is adapted to the rebuilt `trade_rl` architecture merged in commit `8407d664d1d7ea35ccc5ff105cffdb26a3512df6`.

Production remains **NO-GO**.

## Scope adjustment after the residual-core rebuild

The rebuilt repository already removed the legacy direct-action PPO environment, turnover-penalty reward, ignored-action decision timing, curriculum DSR mode, and all compatibility paths. Those items are therefore complete by deletion and must not be reintroduced.
...
This is not a claim that `0.99` is universally optimal. It is a guard against accidentally optimizing only the next one or two decision intervals when the environment contains persistent weights, costs, drawdown state, and compounding.
...
README and architecture documentation state:

- default and minimum residual gamma policy;
- the paired observation additions;
- separate insolvency semantics;
- bootstrap on excess log returns;
- direct-action and DSR changes are already complete by deletion.
