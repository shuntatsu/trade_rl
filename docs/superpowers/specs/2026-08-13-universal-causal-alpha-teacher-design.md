...
Teacher episodes use a chronological split. Complete earlier episodes are used
for teacher selection and BC; the latest complete episode is the causal BC
holdout. An episode's predictor may use only labels whose realization ends
strictly before that episode starts. Predictions carry an explicit knowledge
cutoff, and any prediction whose cutoff is not earlier than its decision index is
rejected.
...
selected configuration digest are persisted before the causal holdout runs.

The latest episode is never consulted during selection. After selection, the
predictor is refitted only on data causally available before that episode, and
the selected controller is evaluated once on the untouched episode.
...
- aggregate confidence bounds plus per-symbol results.
...
- chronological train/holdout episode closure is enforced;
