...
class BinanceFullResearchStages:
    ...
    def _develop(self, work_root: Path) -> ResearchPhaseOutcome:
        ...
        expected_policy_observations = pipeline.policy_observation_count(dataset)
        if policy_observation_count != expected_policy_observations:
            raise RuntimeError(
                f"expected {expected_policy_observations:,} policy observations, "
