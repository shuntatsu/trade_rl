...
明示的な決済専用の成行注文は、約定時に残っている反対方向の保有数量までに制限します。他の注文が先に約定していても、決済注文が新しいポジションを作ることはありません。決済できなくなった注文残量は理由付きで失効し、費用は約定した数量だけにかかります。現在は最小発注額の制限を維持しており、通常の戦略注文からの自動切り替えは未導入です。
