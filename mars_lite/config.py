...
class EnvConfig:
    ...
    obs_risk_state: bool = (
        False  # opt-in: ルール層状態(vol/dd/disagreement scale)を観測に追加
    )
    disagreement_dr_max: float = (
        0.0  # opt-in: 学習中の不一致度ドメインランダム化上限（0で無効）
    )
...
class RunConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "RunConfig":
        ...
        serve_kwargs = {
            k: v for k, v in serve_data.items() if k in ServeConfig.__dataclass_fields__
        }
