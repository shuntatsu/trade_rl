...
class BitgetSource(DataSource):
    ...
    def _fetch_candles(self, symbol: str, granularity: str, step_ms: int) -> pd.DataFrame:
        start_ms = int((self.end - pd.Timedelta(days=self.days)).timestamp() * 1000)
        ...
        for _ in range(400):
            ...
            data = _bitget_get("/api/v2/mix/market/candles", {
                "symbol": symbol, "productType": self.product_type,
                "granularity": granularity,
                "startTime": str(cursor_start), "endTime": str(cursor_end),
                "limit": "1000",
            })
            if not data:
                break
        ...
        if not rows:
            return pd.DataFrame(columns=["timestamp", "open", "high", "low", "close", "volume"])
        df = pd.DataFrame(
            rows, columns=["t", "o", "h", "l", "c", "vbase", "vquote"],
        )
        out = pd.DataFrame({
            "timestamp": pd.to_datetime(df["t"].astype("int64"), unit="ms"),
            "open": df["o"].astype(float), "high": df["h"].astype(float),
    ...
    def load_funding(self, symbol, start=None, end=None):
        ...
        if cache_key in self._kline_cache:
            ...
            for page in range(1, 400):
                data = _bitget_get("/api/v2/mix/market/history-fund-rate", {
                    "symbol": symbol, "productType": self.product_type,
            ...
            if rows:
                df = pd.DataFrame({
                    "timestamp": pd.to_datetime(
                        [int(r["fundingTime"]) for r in rows], unit="ms",
                    ),
                    "funding_rate": [float(r["fundingRate"]) for r in rows],
                }).drop_duplicates("timestamp").sort_values("timestamp").reset_index(drop=True)
                df.to_csv(path, index=False)
...
class OKXSource(DataSource):
    ...
    def _fetch_candles(self, inst_id: str, bar: str) -> pd.DataFrame:
        ...
        for _ in range(400):
            data = _okx_get("/api/v5/market/history-candles", {
                "instId": inst_id, "bar": bar, "after": str(cursor_after), "limit": "100",
            })
            if not data:
                break
            rows.extend(data)
    ...
    def load_funding(self, symbol, start=None, end=None):
        ...
        if cache_key in self._kline_cache:
            ...
            rows: list = []
            for _ in range(400):
                data = _okx_get("/api/v5/public/funding-rate-history", {
                    "instId": inst_id, "after": str(cursor_after), "limit": "100",
                })
                if not data:
                    break
                rows.extend(data)
