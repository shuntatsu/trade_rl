...
class SignalReport:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "top_features": dict(
                sorted(
                    {k: float(v) for k, v in self.per_feature_ic.items()}.items(),
...
def run_trend_gate(
    ...
    agreement = (
        float(np.mean([np.sign(fm) == direction for fm in fold_means]))
        if fold_means
        else 0.0
    )
...
def run_leak_self_test(fs: FeatureSet, horizon: int = 4) -> Dict[str, object]:
    ...
    shuffle_ic = float(
        np.mean([abs(_rank_ic(X[:, j], y_shuf)) for j in range(X.shape[1])])
    )
    ...
    future_ic = float(
        np.mean([abs(_rank_ic(Xs[:, j], ys)) for j in range(Xs.shape[1])])
    )
...
def run_signal_check(
    ...
    passed = (
        mean_ic >= threshold and pos_ratio >= min_positive_ratio and stability_passed
    )
