...
class RiskOverlayEnv(gym.Env):
    ...
    def _refresh_obs_and_cache_action(self) -> np.ndarray:
        alloc_action, _ = self.allocation_agent.predict(
            self._alloc_obs, deterministic=True
        )
        ...
        drawdown = 1.0 - self.inner_env.portfolio_value / max(
            self.inner_env.peak_value, 1e-9
        )
        ...
        # アンサンブル不一致という概念自体が無い）
        return np.array(
            [gross, drawdown, 0.0, vol_ratio, ret_mean, ret_std], dtype=np.float32
        )
    ...
    def step(self, action):
        gross_mult = float(
            np.clip(np.asarray(action, dtype=np.float64).flatten()[0], 0.0, 1.0)
        )
        self._adapter.pending_gross_mult = gross_mult

        obs, reward, terminated, truncated, info = self.inner_env.step(
            self._cached_alloc_action
        )
        self._alloc_obs = obs
