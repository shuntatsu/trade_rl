...
def build_env_kwargs(args, pp, horizon: int = 4) -> dict:
    ...
    if getattr(args, "disagreement_dr", 0.0) > 0:
        ekw["disagreement_dr_max"] = args.disagreement_dr
...
def phase_train(args, output_dir: Path) -> None:
    ...
    if args.lockbox_frac > 0:
        ...
        fs = fs.slice(0, cut)
        print(
            f"[Lockbox] 末尾{args.lockbox_frac:.0%}（{lockbox_fs.n_bars}本）を"
    ...
    if lockbox_fs is not None:
        ...
        report_comparison(
            lockbox_res, lockbox_baselines, "LOCKBOX（最終封印・一度きりの検定）"
        )
        ...
        for bname, bres in lockbox_baselines.items():
            ...
            if not beat:
                lb_passed = False
        print(
            f"[Lockbox GATE] {'PASS' if lb_passed else 'FAIL'} "
...
def phase_wf(args, output_dir: Path) -> None:
    ...
    for cost_mult in [1.0, 2.0]:
        ...
        if report.dsr:
            d = report.dsr
            print(
                f"[Deflated Sharpe] dsr={d['dsr']:.1%} (>=95%推奨) "
...
def phase_overlay(args, output_dir: Path) -> None:
    ...
    overlay_agent = train_risk_overlay(
        train_fs,
        allocation_agent,
        timesteps=args.overlay_timesteps,
        seed=args.seed,
        target_vol=target_vol or 0.5,
    )
    rl_overlay = RLRiskOverlay(overlay_agent, target_vol=target_vol or 0.5)
    rule_overlay = RuleRiskOverlay(
        RuleRiskOverlayConfig(
    ...
    rule_res = evaluate_agent_on_slice(
        allocation_agent, test_fs, post_processor=rule_pp
    )
    ...
    report_comparison(
        rl_res,
        {"rule_overlay": _as_baseline(rule_res, "rule_overlay")},
        "RL overlay vs Rule overlay (allocation agent identical)",
    )
    print(
        "[NOTE] 単一シード・単一シナリオの結果。既定への昇格には"
    ...
    with open(output_dir / "overlay_report.json", "w", encoding="utf-8") as f:
        json.dump(
            {
                "rule_overlay": {
                    k: v for k, v in rule_res.items() if k != "equity_curve"
                },
