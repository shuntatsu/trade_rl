...
class DeploymentGate:
    ...
    def evaluate(self, evidence: DeploymentEvidence) -> GateDecision:
        if evidence.model_version is not None and len(evidence.model_version) > 50:
            return GateDecision(False, "model version exceeds maximum length of 50 characters")
        if evidence.approval_ticket is not None and len(evidence.approval_ticket) > 20:
            return GateDecision(False, "approval ticket exceeds maximum length of 20 characters")
        ...
        if evidence.stage == "canary":
            ...
            if not evidence.model_version or not re.fullmatch(r"[a-zA-Z0-9_\-\.]+", evidence.model_version):
                return GateDecision(False, "canary requires valid model version")
            if not evidence.git_commit or not re.fullmatch(r"[a-fA-F0-9]{40}", evidence.git_commit):
                return GateDecision(False, "canary requires valid 40-character SHA-1 git commit hash")
        ...
        if evidence.stage == "production":
            ...
            if not evidence.git_commit or not re.fullmatch(r"[a-fA-F0-9]{40}", evidence.git_commit):
                return GateDecision(False, "production requires valid 40-character SHA-1 git commit hash")
            if not evidence.drift_report_passed:
                return GateDecision(False, "production requires passing drift report")
            if not evidence.approval_ticket or not re.fullmatch(r"PROD-\d+", evidence.approval_ticket):
                return GateDecision(False, "production requires approval ticket with format PROD-<digits>")
