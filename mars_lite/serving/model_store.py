...
def save_bundle(
    model_dir: Path, name: str, agent: Any, metadata: ModelMetadata
) -> Path:
...
def promote(model_dir: Path, name: str) -> None:
    ...
    _pointer_path(model_dir).write_text(
        json.dumps(
            {"name": name, "promoted_at": time.time()}, indent=2, ensure_ascii=False
        ),
        encoding="utf-8",
    )
...
def rollback(model_dir: Path) -> Optional[str]:
    ...
    _pointer_path(model_dir).write_text(
        json.dumps(
            {"name": previous, "promoted_at": time.time()}, indent=2, ensure_ascii=False
        ),
        encoding="utf-8",
    )
    return previous
