...
class MarketView:
    ...
    @classmethod
    def from_feature_set(cls, fs, t: int, vol_lookback: int = 0,
                         htf_idx: Optional[int] = None) -> "MarketView":
...
class DecisionPipeline:
    ...
    def __init__(self, post_processor=None, min_trade_delta: float = 0.04,
                htf_threshold: float = 0.3, htf_neutral_scale: float = 0.5,
    ...
    def target_weights(self, proj: np.ndarray, state: PortfolioState, market: MarketView):
        ...
        target, pp_info = self.post_processor.process(
            proj, prev, recent_returns=market.recent_returns,
            drawdown=state.drawdown, disagreement=state.disagreement,
        )
