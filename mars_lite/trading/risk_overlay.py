...
class RiskOverlay(Protocol):
    ...
    def scale(
        self, w: np.ndarray, drawdown: float, disagreement: float,
...
class RuleRiskOverlay:
    ...
    def scale(self, w, drawdown, disagreement, recent_returns):
        ...
        # ⑥ アンサンブル不一致縮小
        if disagreement > 0:
            disagreement_scale = float(max(0.0, 1.0 - cfg.disagreement_penalty * disagreement))
...
class RLRiskOverlay:
    ...
    def __init__(self, agent, target_vol: Optional[float] = 0.5,
                bars_per_year: int = BARS_PER_YEAR_1H):
        self.agent = agent
    ...
    def build_obs(self, w, drawdown, disagreement, recent_returns) -> np.ndarray:
        ...
        ret_mean = float(recent_returns.mean()) if recent_returns is not None and len(recent_returns) else 0.0
        ret_std = float(recent_returns.std()) if recent_returns is not None and len(recent_returns) else 0.0
        return np.array(
