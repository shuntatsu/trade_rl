"""Execute sealed portable Canonical M2 Experiment 0004 exactly once."""
...
from research.issue529_portable_exp004 import (
    ...
    _validate_exp004_definition,
    _validate_source_study,
    formal_decision,
    formal_rule_payload,
)
...
def _entry_values(
    ...
    reconstructed = compound(values)
    persisted = _metric(entry, "total_return")
    if not math.isclose(reconstructed, persisted, rel_tol=1e-12, abs_tol=1e-12):
...
def independent_analysis(study_root: Path) -> dict[str, object]:
    ...
    candidate = load_evidence_set(study_root / "experiments/0004/candidate/evidence")
    ...
    if baseline.evidence.ppo_seeds != EXPECTED_SEEDS:
        raise RuntimeError("baseline PPO seed policy drift")
    if candidate.evidence.ppo_seeds != EXPECTED_SEEDS:
        raise RuntimeError("candidate PPO seed policy drift")
    ...
    for strategy in DETERMINISTIC:
        ...
        for symbol_index, symbol in enumerate(EXPECTED_SYMBOLS):
            ...
            baseline_total = compound(before)
            candidate_total = compound(after)
    ...
    for seed in EXPECTED_SEEDS:
        ...
        ppo_by_seed[str(seed)] = by_symbol
    ...
    return {
        ...
        "ppo_effects_by_seed": ppo_by_seed,
...
def crosscheck_persisted_comparison(
    ...
    lightgbm = _mapping(
        cross_symbol.get("lightgbm24"),
        field="persisted lightgbm24 summary",
    )
    formal = _mapping(
        independent.get("lightgbm24_formal_inputs"),
        field="independent lightgbm24 inputs",
    )
    ...
    persisted_median = _number(
        lightgbm.get("median_excess_total_return"),
        field="persisted lightgbm24 median",
    )
    independent_median = _number(
        formal.get("median_excess_total_return"),
        field="independent lightgbm24 median",
    )
    if not math.isclose(
        persisted_median,
        independent_median,
...
def _load_prereg_index(root: Path, *, definition_digest: str, requested_digest: str) -> dict[str, object]:
...
def _prior_binding(state: object) -> tuple[tuple[object, ...], ...]:
    experiments = getattr(state, "experiments", None)
    if not isinstance(experiments, list | tuple) or len(experiments) < 3:
...
def execute(root: Path) -> dict[str, object]:
    ...
    exp4_before = state_before.experiments[3]
    if any(
        value is not None
        for value in (
            exp4_before.candidate,
            exp4_before.verification,
            exp4_before.comparison,
            exp4_before.decision,
            exp4_before.failure,
        )
    ):
        raise RuntimeError("Experiment 0004 is not result-blind before execution")
    ...
    decision = decide_experiment(
        ...
        rationale=(
            "Mechanical replay of the preregistered Experiment 0004 lightgbm24 "
            "decision rule; performance remains interpretation-sealed until "
    ...
    if snapshot.plan.digest != EXPECTED_STUDY_DIGEST:
        raise RuntimeError("Study digest changed during Experiment 0004")
    if snapshot.experiment_sequences != (1, 2, 3, 4):
        raise RuntimeError("Experiment sequence drift after Experiment 0004")
    if snapshot.terminal_sequences != (1, 2, 3, 4):
        raise RuntimeError("Experiment 0004 did not reach one terminal decision")
    if snapshot.frozen:
    ...
    if _prior_binding(state_after) != prior_before:
        raise RuntimeError("prior Experiment evidence changed during Experiment 0004")
    exp4 = state_after.experiments[3]
    if exp4.candidate is None:
        raise RuntimeError("terminal Experiment 0004 lacks candidate evidence")
    if exp4.verification is None or exp4.verification.digest != verification.digest:
        raise RuntimeError("persisted Experiment 0004 verification mismatch")
    if exp4.comparison is None or exp4.comparison.digest != comparison.digest:
        raise RuntimeError("persisted Experiment 0004 comparison mismatch")
    if exp4.decision is None or exp4.decision.digest != decision.digest:
        raise RuntimeError("persisted Experiment 0004 decision mismatch")
    ...
    result_index: dict[str, object] = {
        ...
        "candidate_evidence_fingerprint": exp4.candidate.evidence.fingerprint,
    ...
    (root / "portable-exp004-result-index.json").write_text(
