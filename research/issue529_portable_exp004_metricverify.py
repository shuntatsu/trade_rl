"""Independent deterministic metric seed-invariance verifier for Experiment 0004."""
...
from research.issue529_portable_exp004 import EXPECTED_SEEDS, EXPECTED_SYMBOLS
...
def _metric(run: object, symbol: str, strategy: str, field: str) -> float:
    ...
    matches = [
        ...
        if isinstance(entry, dict) and entry.get("name") == strategy
    ]
    if len(matches) != 1:
    ...
    for seed in seeds[1:]:
        for symbol in symbols:
            for strategy in strategies:
                for field in FIELDS:
                    reference = _metric(
                        runs[reference_seed], symbol, strategy, field
                    )
                    actual = _metric(runs[seed], symbol, strategy, field)
                    if actual != reference:
                        raise RuntimeError(
                            f"seed-sensitive metric {field}: "
...
def build_metric_invariance_report(
    baseline: object,
    candidate: object,
) -> dict[str, object]:
    ...
    return {
        "schema_version": "issue529_exp004_metric_seed_invariance_verification_v1",
        "verified": True,
        "fields": list(FIELDS),
