...
MODULE = "research.issue522_portable_exp003_metricverify"
...
def _module():
    ...
    assert spec is not None, "Experiment 0003 metric verifier is not implemented"
    ...
    for seed in seeds:
        ...
        for symbol in symbols:
            ...
            for strategy in strategies:
                ...
                entries.append({"name": strategy, "metrics": metrics})
...
def _full_pair():
    baseline = _loaded(seeds=FROZEN_SEEDS, symbols=FROZEN_SYMBOLS, strategies=FROZEN_STRATEGIES)
    candidate = _loaded(seeds=FROZEN_SEEDS, symbols=FROZEN_SYMBOLS, strategies=FROZEN_STRATEGIES)
    return baseline, candidate


def test_metric_seed_invariance_accepts_required_metrics() -> None:
    module = _module()
    assert module.assert_metric_seed_invariance(
        _loaded(), seeds=(0, 1), symbols=("BTCUSDT",), strategies=("trend", "mean_reversion")
    ) == 8
...
def test_full_report_requires_1120_checks() -> None:
    module = _module()
...
def test_write_report_is_deterministic_and_performance_blind(tmp_path) -> None:
    module = _module()
    baseline, candidate = _full_pair()
    path, digest = module.write_metric_invariance_report(baseline, candidate, report_root=tmp_path)
    ...
    assert "candidate_total_return" not in text
    assert "formal_decision" not in text
