...
from mars_lite.learning.model_manager import get_model_manager
...
def run_backtest(
    ...
    for ep in range(n_episodes):
        ...
        if verbose >= 1:
            print(
                f"Episode {ep + 1}/{n_episodes}: Reward = {total_reward:.4f}, Length = {length}"
            )
...
def main():
    ...
    if config.use_multi_tf and test_higher:
        env = MarsLiteMultiTFEnv(
            data_1m=test_base, higher_tf_data=test_higher, **env_kwargs
        )
    else:
        single_kwargs = {
            k: v for k, v in env_kwargs.items() if k != "higher_tf_lookback"
        }
