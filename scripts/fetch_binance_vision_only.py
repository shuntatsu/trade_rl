...
from mars_lite.data.binance_vision import (
    fetch_klines_range, fetch_orderflow_vision, fetch_metrics_range,
    fetch_funding_range,
)
...
def main():
    ...
    for symbol in args.symbols:
        ...
        def _kl_progress(i, n, day, nrows):
            if i % 30 == 0 or i == n:
                print(f"    [klines {i}/{n}] {day.strftime('%Y-%m-%d')}: rows={nrows}", flush=True)
        ...
        if len(klines_all):
            for d in range(args.days):
                ...
                sl = klines_all[
                    (klines_all["timestamp"] >= day_ms) & (klines_all["timestamp"] < day_end_ms)
                ]
                if len(sl):
                    save_csv_daily(sl.reset_index(drop=True), output_dir / symbol / "1m", day)
