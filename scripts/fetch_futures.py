...
def _fetch_derivatives_rest(
            ...
            try:
                data = _get(
                    path,
                    {
                        ...
                        "period": period,
                        "startTime": cursor,
                        "endTime": min(cursor + window, end_ms),
                        "limit": 500,
                        **extra,
                    },
                )
    ...
    def to_series(rows, tcol, vcol, cast=float):
        ...
        df = pd.DataFrame(rows)
        return pd.DataFrame(
            {"timestamp": df[tcol].astype("int64"), vcol: df[vcol].astype(cast)}
        ).drop_duplicates("timestamp")

    oi_df = (
        to_series(oi, "timestamp", "sumOpenInterest")
        if oi
        else pd.DataFrame(columns=["timestamp", "sumOpenInterest"])
    )
    ls_df = (
        to_series(ls, "timestamp", "longShortRatio")
        if ls
        else pd.DataFrame(columns=["timestamp", "longShortRatio"])
    )
    tk_df = (
        to_series(taker, "timestamp", "buySellRatio")
        if taker
        else pd.DataFrame(columns=["timestamp", "buySellRatio"])
    )

    out = oi_df.rename(columns={"sumOpenInterest": "open_interest"})
    if not ls_df.empty:
        out = out.merge(
            ls_df.rename(columns={"longShortRatio": "ls_ratio"}),
            on="timestamp",
            how="outer",
        )
    else:
    ...
    if not tk_df.empty:
        ...
        out = out.merge(
            tk_df[["timestamp", "liq_notional"]], on="timestamp", how="outer"
        )
    else:
...
def fetch_derivatives(
    ...
    vision = fetch_metrics_range(
        symbol, start_ms, end_ms, exclude_days=exclude_days, save_cb=save_cb
    )
...
def main():
    ...
    parser.add_argument(
        "--to", type=str, nargs="+", default=["csv"], choices=["csv", "postgres"]
    )
    parser.add_argument(
        "--dsn",
        type=str,
    ...
    for symbol in args.symbols:
        ...
        if "postgres" in args.to and dsn:
            from mars_lite.data.postgres_store import (
                get_existing_derivative_days,
                get_existing_kline_days,
                get_existing_orderflow_days,
            )
            ...
            existing_orderflow_days = get_existing_orderflow_days(
                dsn, "binance", symbol
            )
            ...
            if sym_info:
                ...
                if listing_ms:
                    listing_dt = datetime.fromtimestamp(
                        listing_ms / 1000, tz=timezone.utc
                    )
                    days_listed = (datetime.now(timezone.utc) - listing_dt).days
                    print(
                        f"  listing: {listing_dt.strftime('%Y-%m-%d')} ({days_listed}d ago)"
                    )
        except Exception:
            pass  # 取得失敗は無視して続行
        ...
        print("  derivatives (OI/LS/liq, vision+REST)...")
        derivatives = fetch_derivatives(
            symbol,
            start_ms,
            end_ms,
            exclude_days=existing_deriv_days,
            save_cb=deriv_save_cb,
        )
        ...
        if args.klines_source == "vision":
            from mars_lite.data.binance_vision import (
                fetch_klines_range as fetch_klines_vision,
            )
            ...
            def _kl_progress(i, n, day, nrows):
                if i % 30 == 0 or i == n:
                    ...
                    print(
                        f"  [klines {i}/{n}] {day.strftime('%Y-%m-%d')}: rows={r_str}",
                        flush=True,
                    )
            ...
            for d in range(args.days):
                ...
                klines = fetch_klines_range(
                    symbol, day_ms, min(day_ms + 86_400_000, end_ms)
                )
                if len(klines):
                    if "csv" in args.to:
                ...
                print(
                    f"  [klines {d + 1}/{args.days}] {day.strftime('%Y-%m-%d')}: "
                    f"bars={len(klines)}",
                    flush=True,
                )
        ...
        if not args.skip_orderflow:
            ...
            def _of_progress(i, n, day, nrows):
                if i % 30 == 0 or i == n:
                    r_str = "SKIPPED" if nrows == -1 else str(nrows)
                    print(
                        f"  [orderflow {i}/{n}] {day.strftime('%Y-%m-%d')}: "
                        f"rows={r_str}",
                        flush=True,
                    )
