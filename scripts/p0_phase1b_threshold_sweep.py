...
def main():
    ...
    print("=========================================================================================")
    print(" 構成A 保存済み val_confirm 上での MDD上限閾値スイープ (Sortino>=0.5, Return>=+0.5%)")
    print("=========================================================================================")
    print(f"{'MDD上限':<8} | {'none RL通過(偽陽性)':<16} | {'none Null(正当棄権)':<18} | {'cross RL通過(真陽性)':<18} | {'cross Null(誤棄権)':<16} | {'cross Test中央値':<15} | {'cross Test最悪値':<15}")
    print("-" * 115)
    ...
    for mdd_th in thresholds:
        ...
        print(f"{mdd_th*100:5.0f}%    | {none_rl_pass:>6}/25         | {none_null_sel:>7}/25          | {cross_rl_pass:>7}/25          | {cross_null_sel:>6}/25        | {cross_med:>+10.2%}     | {cross_min:>+10.2%}")
