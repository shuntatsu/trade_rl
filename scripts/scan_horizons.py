...
def main():
    ...
    ap.add_argument(
        "--source", choices=["synthetic", "csv", "hyperliquid"], default="synthetic"
    )
    ...
    if args.source == "synthetic":
        ...
        kwargs = (
            {"data_dir": args.data} if args.source == "csv" else {"days": args.days}
        )
    ...
    print(
        f"Scanning on train slice only: {train_fs.n_bars}/{fs.n_bars} bars "
        f"(test-leak防止のため残り{100 - args.train_fraction * 100:.0f}%は使わない)"
    )

    report = run_horizon_scan(
        train_fs, horizons=tuple(args.horizons), n_folds=args.n_folds, target="raw"
    )
    ...
    if args.breakeven:
        ...
        for r in report.results:
            ...
            be_str = (
                f"{be_ic:.2f}"
                if be_ic is not None
                else "黒字化なし（試した範囲では割に合わない）"
            )
            print(
                f"  horizon={r.horizon:<4} decision_every={de:<3} breakeven_ic={be_str}"
            )
