...
function value(value: number | null, percent = false): string {
  ...
  return percent ? `${value >= 0 ? '+' : ''}${(value * 100).toFixed(2)}%` : value.toFixed(4)
}
...
export function ComparisonInspectorSheet({
  ...
  if (!open) return null
  const point = selectedPoint === null ? model.points.at(-1) ?? null : model.points[selectedPoint] ?? null

  return (
    <aside className="comparison-inspector" role="dialog" aria-modal="true" aria-labelledby="comparison-inspector-title">
      <header>
        <div><span>READ-ONLY DETAIL</span><h2 id="comparison-inspector-title">Comparison inspector</h2></div>
        <button ref={closeRef} type="button" aria-label="Comparison inspectorを閉じる" onClick={onClose}><X size={17} aria-hidden="true" /></button>
      </header>
      <div className="comparison-inspector-tabs" role="tablist" aria-label="Comparison inspector tabs">
        {(['selection', 'metrics', 'config'] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={tab === item} onClick={() => setTab(item)}>{item}</button>)}
      </div>
      ...
      {tab === 'metrics' ? (
        <section role="tabpanel" aria-label="Metrics" className="comparison-inspector-table-wrap">
          <table className="audit-table"><thead><tr><th>Metric</th><th>Left</th><th>Right</th><th>Verdict</th></tr></thead><tbody>
            {model.metricVerdicts.map((metric) => <tr key={metric.key}><td>{metric.label}</td><td>{value(metric.leftValue, metric.key.includes('return') || metric.key.includes('drawdown') || metric.key.includes('cost'))}</td><td>{value(metric.rightValue, metric.key.includes('return') || metric.key.includes('drawdown') || metric.key.includes('cost'))}</td><td data-verdict={metric.verdict}>{metric.verdict}</td></tr>)}
          </tbody></table>
        </section>
      ) : null}

      {tab === 'config' ? (
        <section role="tabpanel" aria-label="Config" className="comparison-inspector-table-wrap">
          ...
          {!comparison.configDifferences.length ? <div className="runtime-empty">設定差はありません。</div> : null}
        </section>
      ) : null}
    </aside>
  )
