...
const comparison = {
  leftResourceId: 'left-resource', rightResourceId: 'right-resource', leftRunId: 'left-run', rightRunId: 'right-run',
  eligibility: { status: 'COMPARABLE', reasons: [], datasetId: 'd'.repeat(64) },
  metrics: [], configDifferences: [],
  folds: [
    { label: 'Fold 1', leftSelectedReturn: .02, leftBaselineReturn: .01, rightSelectedReturn: .04, rightBaselineReturn: .01 },
    { label: 'Fold 2', leftSelectedReturn: .03, leftBaselineReturn: .01, rightSelectedReturn: .05, rightBaselineReturn: .01 },
  ],
  wealth: [
...
function renderWorkspace() {
  ...
  surface.getBoundingClientRect = () => ({ x: 0, y: 0, left: 0, top: 0, right: 1000, bottom: 560, width: 1000, height: 560, toJSON: () => ({}) })
...
describe('InteractiveComparisonWorkspace', () => {
  it('renders synchronized wealth and delta panes with direct labels', () => {
    ...
    expect(screen.getByText('Left baseline')).toBeInTheDocument()
  })

  it('commits a point on click and moves it with ArrowRight', () => {
    const { surface, onCommitPoint } = renderWorkspace()
    fireEvent.pointerDown(surface, { button: 0, pointerId: 1, clientX: 500, clientY: 200 })
    fireEvent.pointerUp(surface, { button: 0, pointerId: 1, clientX: 500, clientY: 200 })
  ...
  it('creates a range only while Range mode is enabled', async () => {
    ...
    fireEvent.pointerDown(surface, { button: 0, pointerId: 2, clientX: 250, clientY: 200 })
    fireEvent.pointerMove(surface, { pointerId: 2, clientX: 750, clientY: 200 })
    fireEvent.pointerUp(surface, { pointerId: 2, clientX: 750, clientY: 200 })
    expect(onCommitRange).toHaveBeenCalledWith(expect.objectContaining({ start: expect.any(Number), end: expect.any(Number) }))
  })
