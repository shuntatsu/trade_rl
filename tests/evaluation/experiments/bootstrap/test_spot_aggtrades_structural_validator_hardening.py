...
def _valid_entry(symbol: str, date: str) -> dict[str, object]:
    ...
    name = url.rsplit("/", 1)[-1]
    timestamp = int(datetime.strptime(date, "%Y-%m-%d").replace(tzinfo=UTC).timestamp() * 1000)
    digest = "a" * 64
    return {
        "symbol": symbol,
        "date": date,
...
def test_nested_archive_report_cannot_gain_economic_field_even_with_new_digest() -> None:
    report = _report()
