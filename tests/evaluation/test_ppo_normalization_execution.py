...
def test_result_digest_without_result_does_not_make_slot_retryable(tmp_path) -> None:
    spec = replication_arm_specs()[0]
    claim_replication_slot(
        tmp_path,
        spec,
    ...
    state = replication_slot_state(tmp_path, spec)
    assert state["consumed"] is True
    assert state["result_published"] is False
    assert state["failed"] is False

    record_consumed_failure(tmp_path, spec, error="crash before result publication")
    failed = replication_slot_state(tmp_path, spec)
    assert failed["consumed"] is True
    assert failed["failed"] is True
