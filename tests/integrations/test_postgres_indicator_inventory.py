...
def test_loads_complete_metadata_inventory_without_npz_payload() -> None:
    database = FakeDatabase()

    inventory = load_postgres_indicator_inventory(database)
...
def test_inventory_identity_is_deterministic_for_reordered_rows() -> None:
    database = FakeDatabase()
    first = load_postgres_indicator_inventory(database)
    database.queries.clear()
    database.artifacts = tuple(reversed(database.artifacts))

    second = load_postgres_indicator_inventory(database)
...
def test_rejects_missing_or_duplicate_artifact_metadata() -> None:
    ...
    with pytest.raises(FileNotFoundError, match="artifact metadata set mismatch"):
        load_postgres_indicator_inventory(missing)
    ...
    with pytest.raises(ValueError, match="duplicate indicator artifact metadata"):
        load_postgres_indicator_inventory(duplicate)


def test_rejects_manifest_digest_and_feature_count_mismatch() -> None:
    manifest_tamper = FakeDatabase()
    row = list(manifest_tamper.manifest)
    row[6] = "0" * 64
    manifest_tamper.manifest = tuple(row)
    with pytest.raises(ValueError, match="feature config digest mismatch"):
        load_postgres_indicator_inventory(manifest_tamper)
    ...
    with pytest.raises(ValueError, match="feature count mismatch"):
        load_postgres_indicator_inventory(feature_count_tamper)


def test_rejects_wrong_manifest_or_artifact_row_contract() -> None:
    manifest = FakeDatabase()
    manifest.manifest = manifest.manifest[:-1]
    with pytest.raises(ValueError, match="manifest row contract"):
        load_postgres_indicator_inventory(manifest)

    artifact = FakeDatabase()
    artifact.artifacts = (artifact.artifacts[0][:-1], *artifact.artifacts[1:])
    with pytest.raises(ValueError, match="artifact metadata row contract"):
        load_postgres_indicator_inventory(artifact)
