...
from trade_rl.learning.dagger import collect_dagger_episode, merge_dagger_rollouts
...
def test_merge_dagger_rollouts_rejects_identity_drift() -> None:
    base = EpisodeSupervisedPolicyDataset(
        observations=np.asarray([[1.0, 1.0]], dtype=np.float32),
        ...
        action_spec_digest=_ACTION_SPEC,
        teacher_config_digest=_TEACHER,
        decision_indices=np.asarray([1], dtype=np.int64),
    ...
    with pytest.raises(ValueError, match="dataset identity"):
        merge_dagger_rollouts(base, (rollout,))
