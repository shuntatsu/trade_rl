...
def test_minimum_notional_noop_is_classified_and_valid() -> None:
    market = _market()
    minimum = np.full_like(market.close, 1_000_000.0)
    constrained = replace(market, minimum_notional=minimum)
    config = _config()
    tape = build_oracle_market_tape(constrained, (0, 3), config.bellman_parameters)
...
def test_transition_uses_per_episode_step_indices() -> None:
    market = _market()
    config = _config()
    tape = build_oracle_market_tape(market, (0, 4), config.bellman_parameters)
    scores = np.zeros((2, 1), dtype=np.float64)
    weights = np.zeros((2, 1, 2), dtype=np.float64)
    targets = np.array([[0.3, -0.2]], dtype=np.float64)
    ...
    for batch_index, step in enumerate((0, 1)):
        single = numpy_transition_step(
            tape=tape,
            step=step,
            prior_scores=scores[batch_index : batch_index + 1],
        ...
        np.testing.assert_array_equal(batched.valid[batch_index], single.valid[0])
        np.testing.assert_allclose(
            batched.close_factor[batch_index], single.close_factor[0]
        )
        np.testing.assert_allclose(
            batched.close_weights[batch_index], single.close_weights[0]
        )
...
def test_transition_accepts_per_episode_targets() -> None:
    market = _market()
    config = _config()
    tape = build_oracle_market_tape(market, (0, 3), config.bellman_parameters)
    scores = np.zeros((2, 1), dtype=np.float64)
    ...
    for batch_index in range(2):
        ...
        np.testing.assert_array_equal(batched.valid[batch_index], single.valid[0])
        np.testing.assert_allclose(
            batched.effective_targets[batch_index], single.effective_targets[0]
        )
