...
def test_market_matrix_fits_only_exogenous_observation_fields() -> None:
    dataset = _dataset()
    matrix = observation_market_matrix(dataset, start=1, stop=5, action_size=3, n_factors=2)
    ...
    assert any(index not in passthrough and normalizer.scale[index] != 1.0 for index in range(normalizer.size))
