...
from tests.serving.helpers import ACTION_NAMES, ACTION_SPEC_DIGEST, OBSERVATION_SIZE, create_bundle
...
def test_baseline_bundle_returns_dynamic_zero_identity_action(tmp_path: Path) -> None:
    runtime = ServingRuntime(
        expected_environment_digest="d" * 64,
        expected_action_names=ACTION_NAMES,
        expected_action_spec_digest=ACTION_SPEC_DIGEST,
        expected_normalizer_digest="9" * 64,
    )
    ...
    np.testing.assert_array_equal(action, np.zeros(len(ACTION_NAMES), dtype=np.float32))


def test_runtime_fails_closed_on_identity_mismatch(tmp_path: Path) -> None:
    runtime = ServingRuntime(expected_environment_digest="f" * 64)
    with pytest.raises(ValueError, match="environment identity"):
        runtime.activate(create_bundle(tmp_path / "bundle"))


def test_runtime_rejects_wrong_shape_nonfinite_and_out_of_bounds_actions(tmp_path: Path) -> None:
    for name, value in (
        ("shape", np.array([0.0])),
        ("finite", np.array([0.0, np.nan, 0.0])),
        ("bounds", np.array([0.0, 1.1, 0.0])),
    ):
        runtime = ServingRuntime(policy_loader=Loader(value))
        runtime.activate(create_bundle(tmp_path / name, policy_mode=PolicyMode.RESIDUAL_POLICY))
