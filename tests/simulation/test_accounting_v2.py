...
def test_economic_insolvency_is_state_not_uncaught_exception() -> None:
    ...
    assert book.termination_reason is EconomicTerminationReason.EXECUTION_COST_EXHAUSTION


def test_maintenance_requirement_causes_margin_call() -> None:
    book = BookState.from_weights(weights=np.array([1.0]), capital=100.0, prices=np.array([100.0]))
    book.set_margin(margin_used=50.0, maintenance_margin=0.25, maintenance_requirement=120.0)
...
def test_split_dividend_and_delisting_settlement_reconcile() -> None:
    book = BookState.from_weights(weights=np.array([0.5]), capital=1_000.0, prices=np.array([100.0]))
    ...
    proceeds = book.settle_positions(mask=np.array([True]), prices=np.array([50.0]), recovery=np.array([0.8]))
