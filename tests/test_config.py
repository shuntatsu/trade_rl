...
def test_postproc_config_measured_defaults():
    ...
    assert (
        cfg.ema_alpha == 1.0
    )  # PostProcessConfig生の既定（make_default_processorが0.5に上書き）
...
def test_run_config_json_roundtrip():
    ...
    assert (
        rc2.serve.guardrails.max_data_age_hours
        == rc.serve.guardrails.max_data_age_hours
    )
