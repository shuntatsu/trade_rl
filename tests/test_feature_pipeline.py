...
from mars_lite.data.sources import CsvSource, SyntheticSource
...
class TestFeaturePipeline:
    def test_shapes(self, feature_set, source):
        fs = feature_set
        n_tf = 4  # 15m, 1h, 4h, 1d
        n_sr_tf = 3  # 1h, 4h, 1d（サポート/レジスタンス特徴）
        n_ichi_tf = 2  # 4h, 1d（一目均衡表の追加TF）
        expected_features = (
            n_tf * len(TF_BLOCK_FEATURES)
            + n_sr_tf * len(SR_BLOCK_FEATURES)
            + n_ichi_tf * len(ICHI_TF_FEATURES)
            + len(BASE_FEATURES)
            + len(CS_FEATURES)
        )
...
class TestFeatureMask:
    def test_mask_zeroes_dropped_features(self, feature_set):
        ...
        np.testing.assert_array_equal(
            masked.features[:, :, 1], feature_set.features[:, :, 1]
        )
        # 形状・レイアウトは維持
