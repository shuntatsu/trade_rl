...
from mars_lite.pipeline.phases import build_post_processor, build_env_kwargs
from mars_lite.trading.post_processor import PostProcessConfig


def _args(**overrides):
    defaults = dict(
        postproc="full", target_vol=0.5, beta_neutral=False,
        min_trade_delta=0.04, lambda_turnover=0.04, htf_gate=False,
        obs_risk_state=False, disagreement_dr=0.0, decision_every=1,
        scan_horizons=False,
    )
...
class TestBuildEnvKwargs:
    ...
    def test_htf_gate_flag_propagates(self):
        ...
        ekw = build_env_kwargs(_args(htf_gate=True), make_default_processor(), horizon=4)
        assert ekw["htf_gate"] is True

    def test_obs_risk_state_flag_propagates(self):
        from mars_lite.trading.post_processor import make_default_processor
        ekw = build_env_kwargs(_args(obs_risk_state=True), make_default_processor(), horizon=4)
    ...
    def test_explicit_decision_every_wins_over_auto(self):
        ...
        ekw = build_env_kwargs(
            _args(decision_every=3, scan_horizons=True), pp, horizon=8,
        )
    ...
    def test_auto_decision_every_from_scan_horizons(self):
        ...
        pp = make_default_processor()
        ekw = build_env_kwargs(
            _args(decision_every=1, scan_horizons=True), pp, horizon=8,
        )
    ...
    def test_no_auto_decision_every_without_scan_horizons(self):
        ...
        pp = make_default_processor()
        ekw = build_env_kwargs(
            _args(decision_every=1, scan_horizons=False), pp, horizon=8,
        )
        assert "decision_every" not in ekw
