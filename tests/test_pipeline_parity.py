...
class TestPipelineParity:
    def test_env_step_matches_external_pipeline_call(self, feature_set):
        ...
        for _ in range(10):
            ...
            market = MarketView.from_feature_set(
                feature_set, t, vol_lookback=pp.cfg.vol_lookback
            )
    ...
    def test_serve_style_call_matches_env_pipeline(self, feature_set):
        ...
        serve_market = MarketView.from_feature_set(
            feature_set, env.t, vol_lookback=pp.cfg.vol_lookback
        )
        serve_target, _ = serve_pipeline.target_weights(
            raw_weights, serve_state, serve_market
        )
        ...
        env_market = MarketView.from_feature_set(
            feature_set, env.t, vol_lookback=pp.cfg.vol_lookback
        )
    ...
    def test_htf_gate_shared_implementation(self, feature_set):
        ...
        env = PortfolioTradingEnv(
            feature_set, episode_bars=50, post_processor=pp, htf_gate=True
        )
        env.reset(seed=0)
