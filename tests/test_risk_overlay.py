...
def _cfg_pair(
    ...
    pp_cfg = PostProcessConfig(
        ...
        dd_derisk_start=dd_derisk_start,
        dd_derisk_floor=dd_derisk_floor,
...
class TestRuleRiskOverlayParity:
    def test_matches_legacy_inline_no_triggers(self):
        ...
        via_overlay = PortfolioPostProcessor(
            pp_cfg, risk_overlay=RuleRiskOverlay(overlay_cfg)
        )
        ...
        w1, info1 = legacy.process(
            raw.copy(), prev.copy(), recent_returns=recent.copy()
        )
        w2, info2 = via_overlay.process(
            raw.copy(), prev.copy(), recent_returns=recent.copy()
        )
    ...
    def test_gross_never_increases(self):
        ...
        overlay = RuleRiskOverlay(
            RuleRiskOverlayConfig(target_vol=0.1, dd_derisk_start=0.05)
        )
        ...
        scaled, _ = overlay.scale(
            w, drawdown=0.3, disagreement=0.5, recent_returns=recent
        )
...
class TestRLRiskOverlay:
    ...
    def test_scale_clips_out_of_range_actions(self):
        ...
        w = np.array([0.5, -0.5])
        scaled, _ = overlay.scale(
            w, drawdown=0.0, disagreement=0.0, recent_returns=None
        )
