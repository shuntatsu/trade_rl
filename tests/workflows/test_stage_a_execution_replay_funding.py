...
def test_v3_replay_rejects_funding_boundary_beyond_evaluation_range(
    tmp_path: Path,
) -> None:
    plan, request = _request()
    candidate_config_digest = plan.candidate("candidate-a").candidate_config_digest
    ...
    out_of_range = request.evaluation_range.stop + 1
    funding = build_funding_evidence_artifact(
        dataset_id=request.dataset_id,
        execution_policy_digest=request.execution_identity,
        symbol_count=1,
        boundaries=(
            FundingBoundaryEvidence(
                processing_index=out_of_range,
                timestamp_ns=out_of_range * 1_000_000_000,
                funding_due=(True,),
