...
def test_transition_end_index_must_advance_beyond_evaluation_start() -> None:
    request = _request(policy=True)
    result = replace(
        _result(policy=True),
        transition_end_indices=(request.evaluation_range.start,),
    )
...
def test_terminal_transition_end_index_must_equal_evaluation_stop() -> None:
    request = _request(policy=True)
    result = replace(
        _result(policy=True),
        transition_end_indices=(request.evaluation_range.stop - 1,),
    ...
    with pytest.raises(ValueError, match="terminal transition end index mismatch"):
        _validate(result, request)
