...
def _canonical_evaluation_dataset_digests(
    ...
    for scope in scope_closure.scopes:
        ...
        if previous != scope.evaluation_dataset_digest:
            raise AssertionError("fixture scope closure has inconsistent dataset identity")
...
def _authoritative_lock():
    ...
    authoritative_lock = closure.build_authoritative_universal_trade_rl_u2_development_lock(
        base_lock=base_lock,
...
def test_u2_authoritative_session_gate_delegates_only_after_exact_lock(
    ...
    result = authority.build_authoritative_universal_trade_rl_u2_development_replay_session(
        authoritative_lock=authoritative_lock,
        base_lock=base_lock,
        manifest=manifest,
        time_partition=partition,
