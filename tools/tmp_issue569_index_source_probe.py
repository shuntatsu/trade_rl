...
ISSUE_NUMBER = 569
SCHEMA_VERSION = "issue569_index_price_structural_probe_v1"
...
def _fetch(url: str) -> tuple[bytes | None, str | None]:
    ...
    for attempt in range(3):
        request = urllib.request.Request(
            url,
            headers={"User-Agent": "trade-rl-issue569-structural-probe/1"},
        )
        try:
...
def _month_grid(month: str) -> tuple[list[int], int]:
    ...
    return [start + offset * INTERVAL_MS for offset in range(days * 24)], days * 24
...
def build_report(*, probe_commit: str) -> dict[str, object]:
    ...
    if (
        len(entries) == 20
        and available == 20
        and checksum_verified == 20
        and len(valid_entries) == 20
        and normalization_compatible
    ):
        status = "PASS_INDEX_SOURCE"
    elif valid_entries and normalization_compatible:
        status = "PARTIAL_INDEX_SOURCE"
    else:
        status = "INCOMPATIBLE_INDEX_SOURCE"
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: probe.py <output-path> <probe-commit>")
    ...
    summary = {
        ...
        "distinct_headers": report["distinct_headers"],
        ...
        "entries": [
            {
                ...
                "missing_grid_rows": entry.get("missing_grid_rows"),
                "spacing_ms": entry.get("spacing_ms"),
                "structurally_valid": entry["structurally_valid"],
