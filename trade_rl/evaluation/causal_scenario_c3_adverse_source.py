...
class C3SourceAdverseEvidence:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            ...
            require_sha256(
                self.source_artifact_digest, field="source_artifact_digest"
            ),
        )
        if not isinstance(self.thresholds, C3AdverseThresholds):
            raise ValueError("thresholds must be C3AdverseThresholds")
        folds = tuple(self.folds)
        if not folds or any(not isinstance(item, C3AdverseFoldEvidence) for item in folds):
...
def _validated_access(
    ...
    *,
    ...
    expected_range = _range_pair(source_fold.get("test_range"), field="source test range")
...
def _validated_top_level_gate(
    ...
    *,
    ...
    if not math.isclose(observed_drawdown, expected_drawdown, rel_tol=0.0, abs_tol=1e-12):
...
def load_c3_source_adverse_evidence(
    ...
    *,
    ...
    experiment_plan_digest = require_sha256(
        _string(
            manifest.get("experiment_plan_digest"), field="experiment_plan_digest"
        ),
    ...
    for raw_fold in raw_folds:
        ...
        if len(required_results) != 1:
            raise ValueError("required scenario is missing or duplicated in source fold")
