...
class OracleBenchmarkResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not steady or any(
            ...
            raise ValueError(
                "steady_seconds must contain finite non-negative values"
            )
        for field, value in (
...
def _measure_seconds(operation: Callable[[], None]) -> float:
...
def run_oracle_teacher_benchmark(
    ...
    *,
    ...
    def operation() -> None:
        _run_legacy_numpy(dataset, resolved_contracts, teacher_config)
    ...
    try:
        ...
        steady_seconds = tuple(
            _measure_seconds(operation) for _ in range(repeat_count)
        )
