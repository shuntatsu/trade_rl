...
def _parse_record(raw_line: bytes) -> "TrainingTelemetryRecord":
    from trade_rl.telemetry.training import TrainingTelemetryRecord

    return TrainingTelemetryRecord.from_json_dict(json.loads(raw_line.decode("utf-8")))
...
def _read_training_telemetry(
    ...
    *,
    ...
    with _telemetry_process_lock(resolved):
        from trade_rl.telemetry.training import TrainingTelemetryPage
...
def _training_telemetry_status(
    path: Path,
) -> "TrainingTelemetryStatus":
    from trade_rl.telemetry.training import TrainingTelemetryStatus
...
class _IndexedTrainingTelemetryWriter:
