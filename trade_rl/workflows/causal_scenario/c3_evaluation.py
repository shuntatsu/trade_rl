...
C3_EVALUATION_REQUEST_SCHEMA: Final = "causal_scenario_c3_evaluation_request_v1"
C3_EVALUATION_RESULT_SCHEMA: Final = "causal_scenario_c3_evaluation_result_v1"
...
def _load_outcome(value: object, *, field: str) -> RealizedPolicyOutcome:
    ...
    _fields(payload, expected, field=field)
...
def _walk_forward_payload(
    ...
    path = source_root / "walk-forward.json"
    raw = _object(json.loads(path.read_text(encoding="utf-8")), field="walk-forward")
...
def execute_c3_evaluation_request(
    ...
    *,
    ...
    """Execute C3 from a published walk-forward run and frozen C2 libraries."""

    request_file = Path(request_path)
    request_root = request_file.parent
    raw = _object(json.loads(request_file.read_text(encoding="utf-8")), field="request")
    _fields(
        raw,
