"""Fail-closed Signal-to-Admission orchestration for Causal Alpha V7."""
...
CAUSAL_ALPHA_V7_RESEARCH_PACKAGE_SCHEMA: Final = "causal_alpha_v7_research_package_v1"
...
def _validate(value: object, *, stage: str) -> _Evidence:
    if not isinstance(getattr(value, "passed", None), bool):
        raise TypeError(f"V7 {stage} evidence has no boolean passed state")
    digest = getattr(value, "digest", None)
    if not isinstance(digest, str):
        raise TypeError(f"V7 {stage} evidence has no digest")
    require_sha256(digest, field=f"V7 {stage} digest")
    if not callable(getattr(value, "to_payload", None)):
        raise TypeError(f"V7 {stage} evidence has no payload")
    return value  # type: ignore[return-value]


class CausalAlphaV7StageRejected(RuntimeError):
    def __init__(self, stage: str, evidence: _Evidence) -> None:
        if stage not in _STAGE_EXIT_CODES:
            raise ValueError("V7 rejected stage is invalid")
        self.stage = stage
        ...
        self.digest = content_digest(
            {
                ...
                "schema_version": f"causal_alpha_v7_{stage}_rejection_v1",
                "stage": stage,
            }
        )
        super().__init__(f"causal alpha V7 {stage} rejected")
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            ...
            "exit_code": self.exit_code,
            "promotion_eligible": False,
            "schema_version": f"causal_alpha_v7_{self.stage}_rejection_v1",
            "stage": self.stage,
        }


@dataclass(frozen=True, slots=True)
class CausalAlphaV7ResearchPackage:
    ...
    schema_version: str = CAUSAL_ALPHA_V7_RESEARCH_PACKAGE_SCHEMA
    digest: str = ""
    ...
    def __post_init__(self) -> None:
        ...
        for name in (
            ...
            require_sha256(getattr(self, name), field=f"V7 package {name}")
        if not self.research_only or self.promotion_eligible:
            raise ValueError("V7 package must remain research-only and non-promotable")
        if self.schema_version != CAUSAL_ALPHA_V7_RESEARCH_PACKAGE_SCHEMA:
            raise ValueError("unsupported V7 package schema")
        object.__setattr__(self, "selected_candidate", candidate)
        expected = content_digest(self.to_payload(include_digest=False))
        if self.digest and self.digest != expected:
            raise ValueError("V7 package digest mismatch")
...
def _persist(
    store: CausalAlphaV7ArtifactStore, stage: str, evidence: _Evidence
) -> None:
    store.write_leaf(
        Path(stage) / "evidence.json",
        store.envelope(
            schema_version=f"causal_alpha_v7_{stage}_envelope_v1",
            evidence_digest=evidence.digest,
...
def _reject(
    ...
    rejection = CausalAlphaV7StageRejected(stage, evidence)
    store.write_leaf(
        "result.json",
        store.envelope(
            schema_version="causal_alpha_v7_terminal_result_v1",
...
def run_universal_causal_alpha_v7_research_pipeline(
    *,
    store: CausalAlphaV7ArtifactStore,
    ...
    """Stop at the first rejection and package only after Admission passes."""

    if not isinstance(store, CausalAlphaV7ArtifactStore):
        raise TypeError("V7 pipeline requires a V7 artifact store")
    ...
    selection = _validate(selection_stage(prepared, signal), stage="selection")
    ...
    raw_config = getattr(selection, "selected_config_digest", None)
    if raw_candidate is None or not isinstance(raw_config, str):
        raise TypeError("V7 passed Selection omitted selected candidate/config")
    candidate = CausalAlphaV7Candidate(raw_candidate)
    require_sha256(raw_config, field="V7 selected config digest")
    if raw_config != store.config_digest:
        raise ValueError("V7 selected config and store identity drifted")
    package = CausalAlphaV7ResearchPackage(
        ...
        selected_config_digest=raw_config,
