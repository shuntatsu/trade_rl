...
class UniversalTradeRLU2DevelopmentSelectionEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.primary_cell_gates, tuple):
            raise TypeError(
                "U2 final Selection primary gates must be an immutable tuple"
            )
        ...
        for result in robustness:
            ...
            if (
                result.robustness_thresholds_digest
                != current_cross_seed_thresholds_digest
            ):
                raise ValueError(
        ...
        if self.digest:
            require_sha256(
                self.digest, field="U2 Development Selection evidence digest"
            )
            if self.digest != expected_digest:
