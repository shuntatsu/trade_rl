...
def build_universal_trade_rl_u2_selection_leaf_metrics(
    *,
    ...
    if training_seed != matching_seeds[0]:
        raise ValueError("U2 Selection leaf training seed/checkpoint identity mismatch")
    if (
...
U2_PAIRED_REPLAY_SCOPE_SCHEMA: Final = "universal_trade_rl_u2_paired_replay_scope_v1"
...
class UniversalTradeRLU2PairedReplayScopeEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.source_window, str) or not self.source_window:
        ...
        if not timestamps or len(timestamps) != len(values):
            raise ValueError(
                "U2 paired replay timestamps/excess must be non-empty and aligned"
            )
...
def build_universal_trade_rl_u2_paired_replay_scope_evidence(
    *,
    ...
    if (
        time_partition.universe_manifest_digest
        != candidate_replay.universe_manifest_digest
    ):
    ...
    if len(candidate_replay.net_simple_returns) != len(candidate_indices) or len(
...
class UniversalTradeRLU2ReducedBootstrapSegment:
    ...
    def __post_init__(self) -> None:
        ...
        paired_scope_evidence = tuple(self.paired_scope_evidence)
        if any(
        ...
        if len({pair.identity for pair in paired_scope_evidence}) != len(
            ...
            raise ValueError(
                "U2 bootstrap segment paired replay identity is duplicated"
            )
        if any(
    ...
    def digest(self) -> str:
        ...
        if self.paired_scope_evidence:
            payload["paired_scope_evidence_digests"] = (
...
def _reduce_u2_paired_replay_evidence_for_windows(
    *,
    ...
    if windows != tuple(
        window for window in U2_DEVELOPMENT_WINDOWS if window in windows
    ):
    ...
    by_window_timestamp: dict[tuple[str, int], dict[int, dict[str, float]]] = (
        defaultdict(lambda: defaultdict(dict))
    )
    for pair in resolved:
    ...
    for source_window in windows:
        ...
        for timestamp in timestamps:
            ...
            if set(seed_map) != set(U2_TRAINING_SEEDS):
                raise ValueError(
                    "U2 paired replay timestamp seed closure is incomplete"
                )
            seed_means: list[float] = []
            for seed in U2_TRAINING_SEEDS:
                symbol_map = seed_map[seed]
                if set(symbol_map) != set(symbols):
                    raise ValueError(
                        "U2 paired replay timestamp symbol closure is incomplete"
                    )
                seed_means.append(fmean(symbol_map[symbol] for symbol in symbols))
            reduced.append(float(median(seed_means)))
        window_pairs = tuple(
...
class UniversalTradeRLU2SeedRobustnessBootstrapResult:
    ...
    def __post_init__(self) -> None:
        ...
        paired_digests = tuple(self.paired_scope_evidence_digests)
    ...
    def __post_init__(self) -> None:
        ...
        if paired_scope_evidence:
            ...
            if paired_digests != self.bootstrap_result.paired_scope_evidence_digests:
                raise ValueError(
                    "U2 seed-robustness bootstrap pairing provenance mismatch"
                )
            ...
            if paired_closure != scope_closure:
                raise ValueError(
                    "U2 seed-robustness paired replay scope closure mismatch"
                )
            if {pair.training_seed for pair in paired_scope_evidence} != set(
                U2_TRAINING_SEEDS
            ):
                raise ValueError(
                    "U2 seed-robustness paired replay seed closure is incomplete"
                )
            if {pair.source_window for pair in paired_scope_evidence} != set(
                expected_windows
            ):
                raise ValueError(
                    "U2 seed-robustness paired replay window closure is incomplete"
                )
...
class UniversalTradeRLU2DevelopmentSelectionEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        for gate in robustness:
            ...
            for pair in gate.paired_scope_evidence:
                if pair.u2_contract_digest != self.u2_contract.digest:
                    raise ValueError(
                        "U2 final Selection paired replay U2 identity mismatch"
                    )
                if pair.time_partition_digest != self.u2_contract.time_partition_digest:
                    raise ValueError(
                        "U2 final Selection paired replay time identity mismatch"
                    )
                ...
                if (
                    ...
                    != checkpoints[pair.training_seed]
                ):
                    raise ValueError(
        ...
        standalone_pairing_digests = tuple(
            pair.digest for gate in (d1, d2) for pair in gate.paired_scope_evidence
        )
