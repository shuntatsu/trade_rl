...
class UniversalTradeRLU2DevelopmentSeedRobustnessEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if seeds != U2_TRAINING_SEEDS:
            raise ValueError(
                "U2 Development seed robustness fixed seed closure drifted"
            )
        ...
        if not math.isclose(
            ...
            raise ValueError(
                "U2 Development robustness seed wealth reduction is inconsistent"
            )
        ...
        if self.all_seed_hard_risk_violation_count != expected_risk_count:
            raise ValueError(
                "U2 Development robustness hard-risk reduction is inconsistent"
            )
        ...
        if len(turnovers) != len(U2_TRAINING_SEEDS) or any(
            ...
            raise ValueError(
                "U2 Development robustness turnover seed closure is invalid"
            )
        ...
        if not math.isclose(
            ...
            raise ValueError(
                "U2 Development robustness turnover reduction is inconsistent"
            )
        ...
        if len(segment_digests) != expected_segment_count:
            raise ValueError(
                "U2 Development robustness bootstrap segment closure is invalid"
            )
        for digest in segment_digests:
            require_sha256(
                digest, field="U2 Development robustness bootstrap segment digest"
            )
        ...
        if reasons != expected_reasons or self.passed != expected_passed:
            raise ValueError(
                "U2 Development robustness pass/reason state is inconsistent"
            )
        object.__setattr__(self, "rejection_reasons", reasons)
...
def evaluate_universal_trade_rl_u2_seed_robustness(
    *,
    ...
    if len(set(identities)) != len(identities):
        raise ValueError(
            "U2 Development seed robustness contains duplicate leaf identity"
        )
    ...
    for seed in U2_TRAINING_SEEDS:
        ...
        for cell in expected_cells:
            ...
            if not cell_leaves:
                raise ValueError(
                    "U2 Development seed robustness seed/cell closure is incomplete"
                )
    ...
    if (
        tuple(segment.source_window for segment in resolved_segments)
