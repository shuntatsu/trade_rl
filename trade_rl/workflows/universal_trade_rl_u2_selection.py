...
U2_PAIRED_REPLAY_SCOPE_SCHEMA: Final = (
...
class UniversalTradeRLU2PairedReplayScopeEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_replay_evidence_digest == self.cash_replay_evidence_digest:
            raise ValueError(
                "U2 paired replay candidate/cash evidence must be distinct"
            )
...
class UniversalTradeRLU2PairedBootstrapSegment(
    ...
    def __post_init__(self) -> None:
        ...
        if not provenance:
            raise ValueError(
                "U2 paired bootstrap segment requires cash pairing provenance"
            )
...
def reduce_universal_trade_rl_u2_paired_replay_evidence(
    *,
    ...
    if len(set(identities)) != len(identities):
        raise ValueError(
            "U2 pairing reducer contains duplicate seed/window/symbol identity"
        )
